...
public:
	...
	void writeInit(void); // writes preset asm code for eq, lt, and gt jumps and where to initially start the code
	void writeLabel(string label); // assembly code that effects the label command
	void writeGoto(string label); // assembly code that effects the goto command
	void writeIf(string label); // assembly code that effects the if-goto command
	void writeCall(string functionName, int numArgs); // assembly code that effects the call command
	void writeReturn(void); // assembly code effects the return command
	void writeFunction(string functionName, int numLocals); // assembly code effects the function command
	...
	void setReturn(int regNum); // Sets location to return to after a jump, stores it in GPR regNum
